...
### Fixed
- `weather_agent._mock_forecast`: `temp_f` was derived from the unrounded `base_c`, causing off-by-0.1 drift when a caller round-tripped  deg C ->  deg F ->  deg C. Now derived from the already-rounded `temp_c`.
